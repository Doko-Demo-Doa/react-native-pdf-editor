...
private fun String.toNitroFieldType(): PdfFieldType = when (this) {
...
class HybridPdfField(private val native: PodofoField) : HybridPdfFieldSpec() {
  ...
  override fun setText(text: String?) { native.text = text }

  override fun isChecked(): Boolean = native.isChecked
  override fun setChecked(checked: Boolean) { native.isChecked = checked }
}
