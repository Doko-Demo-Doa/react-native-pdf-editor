...
internal fun PadesConformanceLevel.toPodofoName(): String = when (this) {
...
internal fun DigestAlgorithm.toPodofoOid(): String = when (this) {
...
@DoNotStrip
class HybridPdfSigningSession(private val native: PodofoSigningWrapper) : HybridPdfSigningSessionSpec() {
  ...
  override fun finishSigningLTA(
    ...
    return Promise.parallel<Unit> {
      native.finishSigningLTA(timestampTokenBase64, certificates?.toList(), crls?.toList(), ocsps?.toList())
    }
  }
  ...
  override fun getOCSPResponderUrl(certificateBase64: String, issuerCertificateBase64: String): String {
    return native.getOCSPFromCertificate(certificateBase64, issuerCertificateBase64)
  }

  override fun buildOCSPRequest(certificateBase64: String, issuerCertificateBase64: String): String {
