...
static PdfStandard14FontType toPodofoStandard14(Standard14FontName name) {
  switch (name) {
    case Standard14FontName::TIMESROMAN: return PdfStandard14FontType::TimesRoman;
    case Standard14FontName::TIMESITALIC: return PdfStandard14FontType::TimesItalic;
    case Standard14FontName::TIMESBOLD: return PdfStandard14FontType::TimesBold;
    case Standard14FontName::TIMESBOLDITALIC: return PdfStandard14FontType::TimesBoldItalic;
    case Standard14FontName::HELVETICA: return PdfStandard14FontType::Helvetica;
    case Standard14FontName::HELVETICAOBLIQUE: return PdfStandard14FontType::HelveticaOblique;
    case Standard14FontName::HELVETICABOLD: return PdfStandard14FontType::HelveticaBold;
    case Standard14FontName::HELVETICABOLDOBLIQUE: return PdfStandard14FontType::HelveticaBoldOblique;
    case Standard14FontName::COURIER: return PdfStandard14FontType::Courier;
    case Standard14FontName::COURIEROBLIQUE: return PdfStandard14FontType::CourierOblique;
    case Standard14FontName::COURIERBOLD: return PdfStandard14FontType::CourierBold;
    case Standard14FontName::COURIERBOLDOBLIQUE: return PdfStandard14FontType::CourierBoldOblique;
    case Standard14FontName::SYMBOL: return PdfStandard14FontType::Symbol;
    case Standard14FontName::ZAPFDINGBATS: return PdfStandard14FontType::ZapfDingbats;
  }
  throw std::invalid_argument("Unknown Standard14FontName");
}

static std::optional<std::string> toOptionalString(const nullable<const PdfString&>& value) {
  if (!value.has_value()) {
    return std::nullopt;
  }
...
std::shared_ptr<HybridPdfPageSpec> HybridPdfDocument::createPage(double width, double height) {
...
std::shared_ptr<HybridPdfFontSpec> HybridPdfDocument::getStandard14Font(Standard14FontName name) {
...
std::shared_ptr<HybridPdfImageSpec> HybridPdfDocument::createImageFromBuffer(const std::shared_ptr<ArrayBuffer>& data) {
  auto image = _doc->CreateImage();
  image->LoadFromBuffer(bufferview(reinterpret_cast<const char*>(data->data()), data->size()));
  PdfImage* rawImage = image.get();
  _images.push_back(std::move(image));
  return std::make_shared<HybridPdfImage>(rawImage);
}

std::shared_ptr<Promise<void>> HybridPdfDocument::save(const std::string& path) {
  auto doc = _doc;
...
void HybridPdfDocument::setTitle(const std::optional<std::string>& title) {
  _doc->GetMetadata().SetTitle(title.has_value() ? nullable<const PdfString&>(PdfString(*title)) : nullptr);
}
...
void HybridPdfDocument::setAuthor(const std::optional<std::string>& author) {
  _doc->GetMetadata().SetAuthor(author.has_value() ? nullable<const PdfString&>(PdfString(*author)) : nullptr);
}
...
void HybridPdfDocument::setSubject(const std::optional<std::string>& subject) {
  _doc->GetMetadata().SetSubject(subject.has_value() ? nullable<const PdfString&>(PdfString(*subject)) : nullptr);
}
